...
CGUIManager::CGUIManager(const Vect2i &resolution)
  ...
  , m_isDisplayedPolyMessage(false)
  , m_firstPolyMessageSeen(false)
  , m_isDisplayedUnlock(false)
...
void CGUIManager::Render (CGraphicsManager *renderManager, CFontManager *fm) {
  if (m_bIsOk) {
    if (m_bLoadedGuiFiles) {
      if (m_drawPushedWindows) {
        int firstWinToDraw = m_PrevWindows.size() - m_drawLastWindows;
        firstWinToDraw = firstWinToDraw < 0 ? 0 : firstWinToDraw;
        for (int i = firstWinToDraw; i < m_PrevWindows.size(); ++i) {
          m_WindowsMap[m_PrevWindows[i]]->Render(renderManager, fm);
        }
        m_WindowsMap[m_sCurrentWindows]->Render(renderManager, fm);
...
void CGUIManager::Update (float elapsedTime) {
  ...
  if (m_bIsOk) {
    ...
    CInputManager *intputManager = INPUTM;
  ...
  if (m_sTransitionEffect.m_bDoEffect && !m_isPaused) {
    switch (m_sTransitionEffect.m_eType) {
      case TE_SHADOW: {
        ...
        float alpha = 0;
        float l_HalfTotalTime = m_sTransitionEffect.m_fTransitionTime * 0.5f;
        if (m_sTransitionEffect.m_fTimeCounter < l_HalfTotalTime) {
          ...
          alpha = 1 - ((l_HalfTotalTime - m_sTransitionEffect.m_fTimeCounter) / l_HalfTotalTime);
...
void CGUIManager::SetChildVisibleGuiElement(const std::string &element, const std::string &child, bool flag) {
  auto it = m_ElementsMap.find(element);
  if (it != m_ElementsMap.end()) {
    it->second->SetChildVisible(child, flag);
  } else {
    LOGGER->AddNewLog(ELL_ERROR, "CGUIManager:: No se ha encontrado el guiElement %s al ejecutar la funcion SetVisibleGuiElement", element.c_str());
  }
}
...
void CGUIManager::ActivatePause(bool input) {
  ...
  m_ElementsMap.find("PauseButtonList")->second->SetVisible(input);
...
void CGUIManager::SetVisibilityMessage(bool input) {
  m_ElementsMap.find("MessageGUI")->second->SetVisible(input);
}

bool CGUIManager::GetIsDisplayedPolyMessage() {
  return m_isDisplayedPolyMessage;
}

void CGUIManager::SetIsDisplayedPolyMessage(bool input) {
  m_isDisplayedPolyMessage = input;
}

bool CGUIManager::GetFirstPolyMessageDisplayed() {
  return m_firstPolyMessageSeen;
}

void CGUIManager::SetFirstPolyMessageDisplayed(bool input) {
  m_firstPolyMessageSeen = input;
}

bool CGUIManager::GetIsDisplayedUnlock() {
  return m_isDisplayedUnlock;
}

void CGUIManager::SetIsDisplayedUnlock(bool input) {
  m_isDisplayedUnlock = input;
}
...
int CGUIManager::GetGUIHeartNumber() {
  return m_GUINumHeart;
}

void CGUIManager::SetGUIHeartNumber(int input) {
  m_GUINumHeart = input;
...
bool CGUIManager::GetIsMainMenu() {
  return m_isMainMenu;
}

void CGUIManager::SetIsMainMenu(bool input) {
  m_isMainMenu = input;
}
...
void CGUIManager::SetIsCromoVisible(std::string _name, bool input) {
  m_ElementsMap.find(_name)->second->SetVisible(input);
}

void CGUIManager::SetTransitionEffect(TransitionEffect transitionEffect) {
  m_sTransitionEffect = transitionEffect;
}
void CGUIManager::StartFade(float time) {
  m_sTransitionEffect.m_bDoEffect = true;

    ...
    if (!parser.LoadFile(initGuiXML.c_str())) {
        ...
        if (m_bIsOk) {
            ...
            if (m.Exists()) {
                ...
                CTexture* Close_normal			= textureM->GetResource(buttonClose_normal);//GetTexture(buttonClose_normal);
            ...
            if (m.Exists()) {
                ...
                m_LifeGUI =	new CLifeGUI(h, w, 300, 200, Vect2f(posx, posy), "", 0, 0, visible, active );
                assert(m_LifeGUI);
                m_LifeGUI->SetName("Life");
                m_LifeGUI->SetBackgroundTexture(t_back, "Background");
            ...
            if (m.Exists()) {
                ...
                CTexture* texture_pointer = textureM->GetResource(texture);//GetTexture(texture);

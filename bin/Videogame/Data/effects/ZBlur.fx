...
{
	//float4 l_Textura1 = float4(tex2D(S0LinearWrapSampler , UV).xyz, 1.0);
	float4 l_DepthMap = float4(tex2D(S1LinearWrapSampler , UV).xyz, 1.0);
	...
	for( int i=0;i<8;i++ )
		l_Color += tex2D(S1LinearWrapSampler,UV + delta[i]*l_PixelInc)*(1-l_Blur)+l_Blur*l_AlbedoColor;

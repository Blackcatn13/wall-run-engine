...
//	SCRIPTM->RunFile(SCRIPTM->GetScriptsMap().find("test2")->second);
  ...
  m_ObjectGUI = new CObject3D(Vect3f(1, 1, 1), 0, 0, 0);
  ...
  m_GUICamera = new CThPSCamera(0.1f, 1000.f, 45.0f * D3DX_PI / 180.0f, 16.f / 9.f, m_ObjectGUI, 10);
  m_FPSCamera = new CFPSCamera(2.5f, 1000.f, 45.0f * D3DX_PI / 180.0f, 16.f / 9.f, m_ObjectFPS);
  ...
  if (!GUIM->GetIsMainMenu()) {
    if (!GUIM->GetIsPaused() && !GUIM->GetIsDisplayedConsole()) {
      ...
      if (ACT2IN->DoAction("TeleportTest4")) {
        PLAYC->SetPosition(Vect3f(328.346, 40.487, -646.076));
        PLAYC->getPhysicController()->SetPosition(Vect3f(328.346, 40.487, -646.076));
      }

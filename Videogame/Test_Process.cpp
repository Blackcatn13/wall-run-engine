...
void CTest_Process::Render()
{
	...
	//CCore::GetInstance()->GetGraphicsManager()->DrawCube(2);
	//CCore::GetInstance()->GetGraphicsManager()->DrawBox(1,2,-1,colRED);
	
	CCore::GetInstance()->GetGraphicsManager()->DrawGrid(5, colBLACK, 10, 10);
	//CCore::GetInstance()->GetGraphicsManager()->DrawPlane(2, Vect3f(0,1,0), 3);
	if(skip < 100)
		skip++;
	else { 
		t1.RotByAngleY(20);
	...
	CCore::GetInstance()->GetGraphicsManager()->SetTransform(t1);
	CCore::GetInstance()->GetGraphicsManager()->DrawCube(0.5, colMAGENTA);
	CCore::GetInstance()->GetGraphicsManager()->DrawSphere(2, colWHITE, 20);
}
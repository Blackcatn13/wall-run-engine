...
CPhysicController *CPhysicsManager::CreateSphericalPhisicController			(float _fRadius, float _fHeight, float _fSlope, float _fSkinwidth, float _fStepOffset
    , ECollisionGroup _uiCollisionGroups, std::string userDataName, const Vect3f &_vPos, float _fGravity, Vect3f color) {
  CPhysicUserData *_pUserData = new CPhysicUserData(userDataName);
  _pUserData->SetPaint(false);
  _pUserData->SetColor(CColor(color.x, color.y, color.z, 1));
  CPhysicController *l_PhysicController = new CPhysicController(_fRadius, _fHeight, _fSlope, _fSkinwidth, _fStepOffset, _uiCollisionGroups, _pUserData, _vPos, _fGravity  );
  PHYSXM->AddPhysicController(l_PhysicController);
  return l_PhysicController;
}

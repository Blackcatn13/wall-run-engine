...
void CCinematicObject::Update(float ElapsedTime) {
  if (m_Playing == true) {
    ...
    if ( ((diff < ElapsedTime) && (diff > -ElapsedTime)) ) {
      Vect3f l_Position = m_CinematicObjectKeyFrames[m_CurrentKeyFrame]->GetPosition();
      float l_Yaw = m_CinematicObjectKeyFrames[m_CurrentKeyFrame]->GetYaw();
      float l_Pitch = m_CinematicObjectKeyFrames[m_CurrentKeyFrame]->GetPitch();
      float l_Roll = m_CinematicObjectKeyFrames[m_CurrentKeyFrame]->GetRoll();
      Vect3f l_Scale = m_CinematicObjectKeyFrames[m_CurrentKeyFrame]->GetScale();

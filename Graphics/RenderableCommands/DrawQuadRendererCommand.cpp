...
{
    CEffectTechnique * l_EffectTechnique = RENDTECHM->GetResource("DrawQuadTechnique")->GetEffectTechnique();
    RECT rect;
    rect.top = 0;
    rect.left = 0;
    for (size_t i = 0; i < m_StageTextures.size(); ++i) {
        RM.EnableAlphaBlend();
        rect.bottom =  m_StageTextures[i].m_Texture->GetHeight();
        rect.right = m_StageTextures[i].m_Texture->GetWidth();

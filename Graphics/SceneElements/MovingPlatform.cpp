...
void CMovingPlatform::MoveToPoint(float dt,  Vect3f point, float minDistance) {
	...
	if (abs(dirRay.x) + abs(dirRay.z) < 4)
	{
		...
		if ((abs(direction.z) + abs(direction.x)) > abs(direction.y))
		{
			...
			if (hit != NULL && hit->getName().substr(0, 6) == "Moving" && info.m_fDistance <= 0.8) {
		...
		else
		{
			...
			if ((l_playerPosition.y > (m_Position.y + l_PosicionMinSobrePlat) && (l_playerPosition.y < (m_Position.y + l_PosicionMaxSobrePlat)) && (hit != NULL && hit->getName().substr(0,6) == "Moving" && info.m_fDistance <= 2.0)))
			{
